...
public class SmartLockForcedOtaTest {
    ...
    public void setUp() throws Exception {
        ...
        if (!f1.exists()) {
            ...
            row.createCell(11).setCellValue("回连异常弹窗内容");
            row.createCell(13).setCellValue("checkupdatetimeout=" + checkupdatetimeout);
            row.createCell(14).setCellValue("updatetimeout=" + updatetimeout);
            row.createCell(15).setCellValue("restart=" + restart);
            row.createCell(26).setCellValue("homebase=" + homebase);
            row.createCell(27).setCellValue("reconnect=" + reconnect);
    ...
    public void test() throws IOException, UiObjectNotFoundException, InterruptedException {
        ...
        while (true) {
            ...
            while(true) {
                ...
                if (mDevice.hasObject(By.res("com.oceanwing.battery.cam:id/dialog_custom_sure").text("Update"))){
                    mDevice.findObject(By.res("com.oceanwing.battery.cam:id/dialog_custom_sure").text("Update")).click();
                    break;
                }else if(homebase){
                    try {
                        ...
                        while (true) {
                            ...
                            if (end - start > reconnect * 1000) {
                                if (mDevice.hasObject(By.text("Drag to lock")) || mDevice.hasObject(By.text("Drag to unlock"))) {
                                    ...
                                    row.createCell(11).setCellValue("没有弹窗");
                                    break;
                                } else {
                                    XLog.i("TIMEOUT");
                            ...
                            }else if (mDevice.hasObject(By.text("Drag to lock")) || mDevice.hasObject(By.text("Drag to unlock"))) {
                                XLog.i("回连成功，没有弹窗");
                                row.createCell(9).setCellValue("PASS");
                                row.createCell(10).setCellValue(retry);
                                row.createCell(11).setCellValue("没有弹窗");
                                break;
                            } else if (mDevice.hasObject(By.res("com.oceanwing.battery.cam:id/dialog_custom_sure").text("Update"))) {
                                XLog.i("回连成功");
                                row.createCell(9).setCellValue("PASS");
                                row.createCell(10).setCellValue(retry);
                                mDevice.findObject(By.res("com.oceanwing.battery.cam:id/dialog_custom_cancel").text("Cancel")).click();
                                break;
